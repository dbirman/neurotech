function RivalrySim_ImmersionSessionMASTER()
    ...
    runTime = 120000; % Simulation will run for 20 seconds
    ...
    function delta = getAdaptationDelta(currVal,excitation,tau)
        ...
        delta = delta+randn()*50.;
        delta = timeStep*delta/tau; % Adjust for the time step and time constant
    end

    %%% This function plots the simulated percept durations
    function plotSimulatedDurations(simRecord)
        resps = simRecord(:,1)>simRecord(:,4);
        resps = resps';
        timestamp = [find(diff([-1 resps]) ~= 0)]; % where does response change
        timestamp(end+1) = size(resps,2);
        changeTimes = timestamp;
        durations = diff(changeTimes); % Find perceptual durations
